...
def hacer_cruce_banco(nomina_rows, pdf_rows, tolerancia, rango_1a, rango_2a):
    ...
    for row in nomina_rows:
        ...
        if match:
            # Si tiene doble abono, sumar TODOS los importes de esa cuenta en los PDFs
            if tiene_doble and clave_norm:
                imp_pdf   = sum(a['imp'] for a in dobles_raw[clave_norm])
                arch_pdfs = " | ".join(set(a['arch'] for a in dobles_raw[clave_norm]))
                match = dict(match)
                match['arch'] = arch_pdfs
            else:
                imp_pdf   = match['imp']
        ...
        resultados.append({
            ...
            'imp_nom':imp_nom,'imp_pdf':imp_pdf if match.get('imp') or tiene_doble else None,
                        ...
                        for pag in p.pages:
                            ...
                            for linea in txt.split("\n"):
                                ...
                                if m:
                                    ...
                                    if imp > 0:
                                        pdf_rows_xl.append(("", m.group(1), imp, "", "", f.name))
                except Exception as ep:
