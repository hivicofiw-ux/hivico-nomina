...
# ── Parsers PDFs bancarios (lógica exacta de 05_extraer_pdfs_banco.py) ────
    ...
    for linea in texto.split('\n'):
        ...
        if m:
            ...
            if imp and imp > 0:
                ...
                regs.append({'cuenta': m.group(1),
                             'nombre': nm.group(1).strip() if nm else '',
                             'importe': imp, 'estatus': 'OPERACION EXITOSA'})
    return regs
...
def _parsear_banorte_normal(texto):
    ...
    for linea in texto.split('\n'):
        ...
        if m:
            ...
            craw = m.group(3)
            craw = craw.rstrip('0') if craw.endswith('0') else craw
            cta  = craw.lstrip('0')
            if imp and imp > 0 and cta:
                regs.append({'cuenta': cta, 'nombre': '',
                             'importe': imp, 'estatus': 'OPERACION EXITOSA'})
    return regs
...
def _parsear_banorte_v2(texto):
    ...
    for linea in texto.split('\n'):
        ...
        cuentas = re.findall(r'\b(\d{10})\b', linea[m.end():])
...
def _parsear_banorte_individual(texto):
    regs = []
    cuenta = imp = None; estatus = 'OPERACION EXITOSA'
    for linea in texto.split('\n'):
        linea = linea.strip()
        m = re.search(r'Número Cuenta / Tarjeta Abono:\s*(\d{10})', linea)
        if m: cuenta = m.group(1)
        m = re.search(r'^Importe:\s*([\d,]+\.\d{2})', linea)
        if m: imp = _limpiar_imp(m.group(1))
        m = re.search(r'Descripción de Resultado:\s*(.+)', linea)
        if m: estatus = 'OPERACION EXITOSA' if 'APLIC' in m.group(1).upper() else 'RECHAZADO'
        if cuenta and imp:
            regs.append({'cuenta': cuenta, 'nombre': '',
                         'importe': imp, 'estatus': estatus})
            cuenta = imp = None; estatus = 'OPERACION EXITOSA'
    return regs
...
def _parsear_santander(texto):
    ...
    for linea in texto.split('\n'):
        ...
        m = re.search(
            ...
            r'(Procesado|Rechazado|PROCESADO|RECHAZADO|no\b)', linea)
        if m:
            imp = _limpiar_imp(m.group(3))
            if imp and imp > 0:
                est_raw = m.group(5).upper()
                regs.append({'cuenta': m.group(1),
                             'nombre': re.sub(r'\s*[-—]+\s*',' ',m.group(4)).strip(),
                             'importe': imp,
                             'estatus': 'RECHAZADO' if est_raw == 'NO' else est_raw})
    return regs

def procesar_pdf_banco(contenido_bytes, nombre_archivo):
    """
    Procesa un PDF bancario usando los parsers específicos por banco.
    Retorna lista de tuplas (banco, cuenta, importe, nombre, estatus, archivo).
    """
    ...
    if banco == 'BBVA':
        ...
        if 'Número Cuenta / Tarjeta Abono' in texto:
            regs = _parsear_banorte_individual(texto)
        elif '072 01' in texto:
            regs = _parsear_banorte_normal(texto)
        elif len(texto.strip()) > 100:
        ...
        # Banco desconocido — regex genérico de respaldo
        for linea in texto.split('\n'):
            ...
            if m:
                ...
                if imp > 0:
                    regs.append({'cuenta': m.group(1), 'nombre': '',
                                 'importe': imp, 'estatus': 'OPERACION EXITOSA'})

    resultado = [
        (banco or '', r['cuenta'], r['importe'],
         r['nombre'], r['estatus'], nombre_archivo)
        for r in regs
        if r.get('importe') and r['importe'] > 0
    ]
...
def hacer_cruce_banco(nomina_rows, pdf_rows, tolerancia, rango_1a, rango_2a):
    ...
    for row in nomina_rows:
        ...
        tiene_doble=(norma_c(cuenta or '') in dobles_raw or norma_c(clabe or '') in dobles_raw)
        resultados.append({
            'qna':qna_nom,'quincena':quincena or '',
            'nss':nss or '','nombre':nombre or '',
            'banco':banco_e or '','cuenta':cuenta or '','clabe':clabe or '',
            'imp_nom':imp_nom,'imp_pdf':match.get('imp') if match.get('imp') else None,
            'diff':diff,'banco_pdf':match['banco'],'nombre_pdf':match['nombre'],
            'estatus_pdf':match['estatus'],'arch_pdf':match['arch'],
...
<div class="sec-hdr">
  ...
  <span class="sec-desc">Lectura, procesamiento y carga · Q1 y Q2</span>
    ...
    with col1:
        ...
        ups_q1 = st.file_uploader("Q1", type=["xlsx","xls"],
    ...
    with col2:
        ...
        ups_q2 = st.file_uploader("Q2", type=["xlsx","xls"],
                                   accept_multiple_files=True,
    ...
    if ups_q1 or ups_q2:
        if st.button("▶  Procesar sábanas", type="primary", use_container_width=True):
            dfs, log_det = [], []
            total_arch = len(ups_q1 or []) + len(ups_q2 or [])
            bar = st.progress(0)
            i = 0
            for f in (ups_q1 or []):
                df, err = leer_sabana_bytes(f.read(), f.name, 1)
                if err:
                    log_det.append(("ERR", f.name, "Q1", 0, 0.0, err))
                    st.error(f"Q1 · {f.name}: {err}")
                elif df is not None and len(df) > 0:
                    dfs.append(df)
                    sc = int(df["clave_interbancaria"].isna().sum())
                    log_det.append(("OK", f.name, "Q1", len(df), df['total_quincenal'].sum(),
                                    f"Hoja: {df['hoja_detectada'].iloc[0]}  ·  Sin CLABE: {sc}"))
                i += 1; bar.progress(i / total_arch)
            for f in (ups_q2 or []):
                df, err = leer_sabana_bytes(f.read(), f.name, 2)
                if err:
                    log_det.append(("ERR", f.name, "Q2", 0, 0.0, err))
                    st.error(f"Q2 · {f.name}: {err}")
                elif df is not None and len(df) > 0:
                    dfs.append(df)
                    sc = int(df["clave_interbancaria"].isna().sum())
                    log_det.append(("OK", f.name, "Q2", len(df), df['total_quincenal'].sum(),
                                    f"Hoja: {df['hoja_detectada'].iloc[0]}  ·  Sin CLABE: {sc}"))
                i += 1; bar.progress(i / total_arch)
            bar.empty()
            if dfs:
            ...
            if not ups_banco:
                ...
                if not nom_rows:
                    if "df_sabanas" in st.session_state:
                        ...
                        fuente_nom = f"Sábanas Módulo 1 ({len(nom_rows):,} empleados)"
                    else:
                ...
                for idx_f, f in enumerate(ups_banco):
                    contenido = f.read()
                    regs, err = procesar_pdf_banco(contenido, f.name)
                    if err:
                        errs_pdf.append(f"{f.name}: {err}")
                    else:
                        pdf_rows_local.extend(regs)
                        if not regs:
                            errs_pdf.append(f"⚠ {f.name}: 0 registros — verifica el formato")
                    bar.progress((idx_f+1)/len(ups_banco))
                bar.empty()

                if not pdf_rows_local:
                    st.error("❌ No se encontraron registros en los PDFs.")
            ...
            if "df_sabanas" in st.session_state:
                ...
                if up_sabs:
                    st.markdown(f'<div class="up-ok">📎 {len(up_sabs)} archivo(s)</div>',
                                unsafe_allow_html=True)
        with cy:
            st.markdown('<div class="up-lbl">PDFs de dispersión bancaria</div>',
                        unsafe_allow_html=True)
            up_pdfs = st.file_uploader("PDFs bancarios", type=["pdf"],
                                        accept_multiple_files=True,
                                        key="up_banco2", label_visibility="collapsed")
            if up_pdfs:
                st.markdown(f'<div class="up-ok">📎 {len(up_pdfs)} PDF(s)</div>',
                            unsafe_allow_html=True)
        ...
        if st.button("▶  Preparar datos", use_container_width=True,
            ...
            for idx3, f in enumerate(up_pdfs):
                ...
                if err:
                    ...
                    if not regs:
                        errs.append(f"⚠ {f.name}: 0 registros — verifica el formato del PDF")
