...
def hacer_cruce_banco(nomina_rows, pdf_rows, tolerancia, rango_1a, rango_2a):
    ...
    # Agrupar empleados en múltiples sábanas — mismo NSS+cuenta = 1 fila
...
def exportar_nomina_pdf_excel(resultados,stats,dobles_raw,totales_pdf,empresa,reg_pat,periodo,fecha_pago,tolerancia):
    ...
    def hoja_det(wb_o,regs,nom,bg_h):
        ...
        for r in sorted(regs,key=lambda x:(x['banco'],x['nombre'])):
            est=r['estatus']
            _obs_v = r.get('obs','') or ''
            if r['doble_abono'] and not _obs_v: _obs_v = '⚠ Doble abono en PDF'
  ...
  <div class="kpi k-blue">
    ...
    <div class="kpi-s">Q1 + Q2</div>
        ...
        try:
            ...
            st.warning(f"No se pudo generar Excel: {_e_xl}")
    ...
    if PSYCOPG2_OK and usar_bd:
        ...
        if st.button("▶  Cargar nómina y archivo de dispersión",
            ...
            if not ups_banco:
                ...
                # ── Paso 2: leer archivo dispersión (CSV/Excel) ───
                pdf_rows_local = []
                errs_pdf = []
                ...
                for idx_f, f in enumerate(ups_banco):
                    try:
                        ...
                        if f.name.lower().endswith(".csv"):
                            import csv as _c
                            for _r in _c.DictReader(raw.decode("utf-8-sig").splitlines()):
                                try:
                                    _imp = float(_r.get("importe",0) or 0)
                                    if _imp>0: pdf_rows_local.append(("",_r.get("cuenta",""),_imp,_r.get("nombre",""),_r.get("banco",""),_r.get("archivo_pdf","") or f.name))
                                except: pass
                        else:
                            import openpyxl as _ox
                            _wb=_ox.load_workbook(io.BytesIO(raw),data_only=True,read_only=True)
                            _ws=_wb.active
                            _rows=list(_ws.iter_rows(values_only=True))
                            # Detectar fila header
                            _hi=0
                            for _ri,_rw in enumerate(_rows[:5]):
                                if any("CUENTA" in str(v or "").upper() for v in _rw): _hi=_ri; break
                            _hdr=[str(v or "").upper().strip() for v in _rows[_hi]]
                            _ci=lambda k: next((i for i,h in enumerate(_hdr) if k in h),None)
                            _cb,_cc,_ci2,_cn,_ca=_ci("BANCO"),_ci("CUENTA"),_ci("IMPORTE"),_ci("NOMBRE"),_ci("ARCHIVO")
                            for _rw in _rows[_hi+1:]:
                                if not _rw or not any(_rw): continue
                                try:
                                    _imp=float(_rw[_ci2] or 0) if _ci2 is not None else 0
                                    if _imp<=0: continue
                                    pdf_rows_local.append(("",str(_rw[_cc] or "") if _cc is not None else "",_imp,str(_rw[_cn] or "") if _cn is not None else "",str(_rw[_cb] or "") if _cb is not None else "",str(_rw[_ca] or f.name) if _ca is not None else f.name))
                                except: pass
                            _wb.close()
                ...
                if not pdf_rows_local:
                    st.error("❌ Sin registros. Sube el CSV/Excel de extraer_pdfs.py")
                else:
        ...
        if st.button("▶  Preparar datos", use_container_width=True,
            ...
            for idx3, f in enumerate(up_pdfs):
                try:
                    ...
                    if f.name.lower().endswith(".csv"):
                        ...
                        _ws3=_wb3.active
                        _rows3=list(_ws3.iter_rows(values_only=True))
                        _hi3=0
            ...
            if not pdf_rows_xl:
                st.error("❌ Sin registros. Sube el CSV/Excel de extraer_pdfs.py"); st.stop()
