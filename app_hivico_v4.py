...
def _parsear_bbva(texto):
    ...
    for linea in texto.split('\n'):
        ...
        if m:
            ...
            if imp and imp > 0:
                nm = re.search(r'CHEQUES\s+[_\s]*[\d,.]+\s+(.+?)\s+OP\s+OPERACION', linea)
                regs.append(('', m.group(1), imp, nm.group(1).strip() if nm else '', 'BBVA'))
    return regs
...
def _parsear_banorte_normal(texto):
    ...
    for linea in texto.split('\n'):
        ...
        if m:
            imp = _limpiar_imp(m.group(1)); cta = m.group(3).lstrip('0')
...
def _parsear_banorte_v2(texto):
    ...
    for linea in texto.split('\n'):
        ...
        resto = linea[m.end():]
        mb = re.search(r'(?:Banorte|BANORTE)\s+(\d{10})', resto)
        cta = mb.group(1) if mb else (re.findall(r'\b(\d{10})\b', resto)[1:2] or re.findall(r'\b(\d{10})\b', resto)[:1] or [None])[0]
        if cta: regs.append(('', cta, imp, '', 'BANORTE'))
    return regs

def _parsear_banorte_individual(texto):
    regs, cuenta, imp = [], None, None
    for linea in texto.split('\n'):
        linea = linea.strip()
        ...
        if cuenta and imp:
            regs.append(('', cuenta, imp, '', 'BANORTE')); cuenta = imp = None
    return regs

def _parsear_santander(texto):
    regs = []
    for linea in texto.split('\n'):
        m = re.search(r'^(\d{8,14})\s+(\d+\s?\d+)\s+\$([\d,]+\.\d{2})\s+(.+?)\s+(Procesado|Rechazado|PROCESADO|RECHAZADO|no\b)', linea.strip())
        if m:
...
def _ocr_pdf(bytes_data, banco):
    try:
        ...
        ] if p and _os.path.isfile(_os.path.join(p, "pdftoppm.exe"))), None)
        kw = {"poppler_path": poppler_path} if poppler_path else {}
        imgs = convert_from_bytes(bytes_data, dpi=300, **kw)
        if banco == 'BBVA':
            regs = []
            for img in imgs:
                texto = pytesseract.image_to_string(img, lang='eng', config='--oem 3 --psm 6')
                lineas, buf = [], ""
                for l in texto.split('\n'):
                    l = l.strip()
                    if not l: continue
                    if re.search(r'\d{10}', l):
                        if buf: lineas.append(buf)
                        buf = l
                    else:
                        buf = (buf + " " + l).strip() if buf else l
                if buf: lineas.append(buf)
                regs.extend(_parsear_bbva("\n".join(lineas)))
            return regs
        elif banco == 'SANTANDER':
            texto = "\n".join(pytesseract.image_to_string(img, lang='eng', config='--oem 3 --psm 6') for img in imgs)
            return _parsear_santander(texto)
...
def _procesar_pdf_bytes(bytes_data, nombre_archivo):
    ...
    try:
        ...
        if banco == 'BBVA':
            ...
            if len(regs) < 5: regs = _ocr_pdf(bytes_data, 'BBVA')
        elif banco == 'BANORTE':
            ...
            if not regs: regs = _parsear_banorte_normal(texto) or _parsear_banorte_v2(texto)
        elif banco == 'SANTANDER':
            regs = _parsear_santander(texto)
            if len(regs) < 3: regs = _ocr_pdf(bytes_data, 'SANTANDER')
        if not regs:
            for linea in texto.split('\n'):
                m = re.search(r'(\d{10,18})\s+.*?\$?([\d,]{1,15}\.\d{2})', linea)
                if m:
                    imp = _limpiar_imp(m.group(2))
                    if imp and imp > 0: regs.append(('', m.group(1), imp, '', banco))
    except Exception: pass
...
def hacer_cruce_banco(nomina_rows, pdf_rows, tolerancia, rango_1a, rango_2a):
    ...
    # ── Agrupar empleados que aparecen en múltiples sábanas ──────────
    # Mismo NSS+cuenta → sumar importes y registrar todos los archivos
    from collections import OrderedDict
    grupos = OrderedDict()
    for row in nomina_rows:
        nss,nombre,cuenta,clabe,banco_e,imp_nom,quincena,arch_orig=row
        imp_nom=float(imp_nom or 0)
        clave_nom = (norma_c(nss or ''), norma_c(cuenta or '') or norma_c(clabe or ''))
        if clave_nom not in grupos:
            grupos[clave_nom] = {
                'nss':nss,'nombre':nombre,'cuenta':cuenta,'clabe':clabe,
                'banco_e':banco_e,'imp_nom':imp_nom,'quincena':quincena,
                'archivos':[arch_orig or ''], 'n_sabanas':1,
            }
        else:
            grupos[clave_nom]['imp_nom']    += imp_nom
            grupos[clave_nom]['n_sabanas']  += 1
            if arch_orig and arch_orig not in grupos[clave_nom]['archivos']:
                grupos[clave_nom]['archivos'].append(arch_orig)
            # Actualizar datos si estaban vacíos
            if not grupos[clave_nom]['nombre'] and nombre:
                grupos[clave_nom]['nombre'] = nombre
            if not grupos[clave_nom]['cuenta'] and cuenta:
                grupos[clave_nom]['cuenta'] = cuenta
            if not grupos[clave_nom]['clabe'] and clabe:
                grupos[clave_nom]['clabe'] = clabe

    resultados=[]; stats=defaultdict(int)
    for clave_nom, g in grupos.items():
        nss     = g['nss']
        nombre  = g['nombre']
        cuenta  = g['cuenta']
        clabe   = g['clabe']
        ...
        imp_nom = g['imp_nom']
        quincena= g['quincena']
        arch_orig = ' | '.join(g['archivos'])
        n_sabanas = g['n_sabanas']
        ...
        obs = f"En {n_sabanas} sábanas: {arch_orig}" if n_sabanas > 1 else ""
        resultados.append({
            ...
            'doble_abono':tiene_doble,'n_sabanas':n_sabanas,'obs':obs,
        })
...
def exportar_nomina_pdf_excel(resultados,stats,dobles_raw,totales_pdf,empresa,reg_pat,periodo,fecha_pago,tolerancia):
    ...
    # RESUMEN usa fórmulas que apuntan a DETALLE COMPLETO
    # Las filas de datos en DETALLE COMPLETO empiezan en 5 (título+subtítulo+nota+header+datos)
    det_sheet = "DETALLE COMPLETO"
    n_det = len(resultados)
    ltr_n = get_column_letter(hdrs_det.index("IMPORTE NÓMINA")+1)
    ltr_p = get_column_letter(hdrs_det.index("IMPORTE PDF")+1)
    ltr_e = get_column_letter(hdrs_det.index("ESTATUS")+1)
    f_ini, f_fin = 5, 5+n_det-1  # filas de datos en DETALLE
    _ok_f  = f"=CONTAR.SI('{det_sheet}'!{ltr_e}{f_ini}:{ltr_e}{f_fin},"+'"OK")'
    _dif_f = f"=CONTAR.SI('{det_sheet}'!{ltr_e}{f_ini}:{ltr_e}{f_fin},"+'"DIFIERE")'
    _no_f  = f"=CONTAR.SI('{det_sheet}'!{ltr_e}{f_ini}:{ltr_e}{f_fin},"+'"NO EN PDF")'
    _pct_f = f"=CONTAR.SI('{det_sheet}'!{ltr_e}{f_ini}:{ltr_e}{f_fin},"+f'"OK")/{n_det}*100'
    ws1.append(["TOTAL GENERAL",
                f"=SUMA('{det_sheet}'!{ltr_n}{f_ini}:{ltr_n}{f_fin})",
                f"=SUMA('{det_sheet}'!{ltr_p}{f_ini}:{ltr_p}{f_fin})",
                f"=SUMA('{det_sheet}'!{ltr_n}{f_ini}:{ltr_n}{f_fin})-SUMA('{det_sheet}'!{ltr_p}{f_ini}:{ltr_p}{f_fin})",
                _ok_f, _dif_f, _no_f, _pct_f,
                len(dobles_raw),""])
    fr=ws1.max_row
    ...
    # Posiciones de columnas clave (1-based)
    COL_NOM  = hdrs_det.index("IMPORTE NÓMINA") + 1   # H
    COL_PDF  = hdrs_det.index("IMPORTE PDF") + 1       # I
    COL_DIFF = hdrs_det.index("DIFERENCIA") + 1        # J
    COL_EST  = hdrs_det.index("ESTATUS") + 1           # O
    LTR_NOM  = get_column_letter(COL_NOM)
    LTR_PDF  = get_column_letter(COL_PDF)
    LTR_DIFF = get_column_letter(COL_DIFF)
    LTR_EST  = get_column_letter(COL_EST)
    TOL      = tolerancia  # usar tolerancia del cruce
    ...
    def hoja_det(wb_o,regs,nom,bg_h):
        ...
        n_regs = len(regs)
        # Filas de datos: 4 .. 4+n_regs-1
        fila_ini = 4
        fila_fin = fila_ini + n_regs - 1

        xl_titulo(ws,f"{nom} — NÓMINA vs PDF",1,len(hdrs_det),bg_h)
        ws.merge_cells(f"A2:{get_column_letter(len(hdrs_det))}2")
        # Subtítulo con fórmulas dinámicas
        ws["A2"].value=(f"Total: {n_regs:,}  |  Edita NÓMINA $ e IMPORTE PDF para recalcular")
        ws["A2"].font=xl_fn(bold=True,size=9,color=bg_h)
        ws["A2"].alignment=Alignment(horizontal="center")

        # Nota de celdas editables
        ws.merge_cells(f"A3:{get_column_letter(len(hdrs_det))}3")
        ws["A3"].value = (f"🔵 NÓMINA $ y PDF $ son editables  |  "
                          f"DIFERENCIA y ESTATUS se recalculan automáticamente  |  "
                          f"Tolerancia: ${TOL:.2f}")
        ws["A3"].font = Font(size=8, italic=True, color="1F4E79", name="Calibri")
        ws["A3"].fill = xl_fill("EEF4FF")
        ...
        # Colorear encabezados editables en azul
        ws.cell(fr, COL_NOM).fill  = xl_fill("1A5276")  # azul oscuro = editable
        ws.cell(fr, COL_PDF).fill  = xl_fill("1A5276")
        ws.cell(fr, COL_DIFF).fill = xl_fill("0F3460")  # más oscuro = fórmula
        ws.cell(fr, COL_EST).fill  = xl_fill("0F3460")

        regs_sorted = sorted(regs, key=lambda x:(x['banco'],x['nombre']))
        for idx_r, r in enumerate(regs_sorted):
            est=r['estatus']
            obs_val = r.get('obs','') or ''
            if r['doble_abono'] and not obs_val: obs_val = '⚠ Doble abono en PDF'
            fila = fila_ini + idx_r

            # Valores base (sin DIFERENCIA ni ESTATUS — van con fórmula)
            imp_nom_val = r['imp_nom'] if r['imp_nom'] else 0
            imp_pdf_val = r['imp_pdf'] if r['imp_pdf'] else ""

            row=[r['qna'],r['quincena'],r['nss'],r['nombre'],r['banco'],r['cuenta'],r['clabe'],
                 imp_nom_val, imp_pdf_val,
                 None,  # DIFERENCIA — se pondrá fórmula abajo
                 r['banco_pdf'],r['nombre_pdf'],r['estatus_pdf'],
                 r['metodo'],
                 None,  # ESTATUS — se pondrá fórmula abajo
                 '⚠ DOBLE' if r['doble_abono'] else '',r['arch_origen'],r['arch_pdf'],
                 obs_val]
            ...
            # ── NÓMINA $ → editable, azul ─────────────────────────
            cn = ws.cell(fr, COL_NOM)
            cn.value = imp_nom_val
            cn.number_format = '$#,##0.00'
            cn.alignment = Alignment(horizontal="right")
            cn.font = Font(bold=True, color="1A3A6B", size=9, name="Calibri")
            cn.fill = xl_fill("D6EAF8")  # azul claro = editable

            # ── PDF $ → editable, azul ────────────────────────────
            cp = ws.cell(fr, COL_PDF)
            if imp_pdf_val != "":
                cp.value = imp_pdf_val
                cp.number_format = '$#,##0.00'
                cp.alignment = Alignment(horizontal="right")
                cp.font = Font(bold=True, color="1A3A6B", size=9, name="Calibri")
                cp.fill = xl_fill("D6EAF8")  # azul claro = editable
            else:
                cp.value = ""
                cp.fill = xl_fill("F2F2F2")

            # ── DIFERENCIA → fórmula automática ───────────────────
            cd = ws.cell(fr, COL_DIFF)
            cd.value = f'=SI({LTR_PDF}{fr}="","",{LTR_NOM}{fr}-{LTR_PDF}{fr})'
            cd.number_format = '$#,##0.00;($#,##0.00);"-"'
            cd.alignment = Alignment(horizontal="right")
            cd.font = Font(size=9, name="Calibri")
            cd.fill = xl_fill("FFF9E6")  # amarillo muy claro = fórmula

            # ── ESTATUS → fórmula automática ─────────────────────
            ce2 = ws.cell(fr, COL_EST)
            ce2.value = (
                f'=SI({LTR_PDF}{fr}="","NO EN PDF",'
                f'SI(ABS({LTR_DIFF}{fr})<={TOL},"OK",'
                f'"DIFIERE"))'
            )
            ce2.font = xl_fn(bold=True, size=8)
            ce2.alignment = Alignment(horizontal="center")
            # Color del estatus también dinámico (basado en valor calculado al abrir)
            # Ponemos color por el estatus original hasta que abran en Excel
            ce2.fill = xl_fill(CV if est=='OK' else CR if est=='NO EN PDF' else CA)

            if r['doble_abono']:
                cd2=hdrs_det.index('DOBLE ABONO')+1
                ws.cell(fr,cd2).fill=xl_fill("FF0000")
                ws.cell(fr,cd2).font=xl_fn(bold=True,size=8,color="FFFFFF")

        # ── FILA DE TOTALES con fórmulas SUMA ─────────────────────
        ft = fila_fin + 1
        ws.cell(ft,1,"TOTALES").font=xl_fn(bold=True,size=10)
        for j in range(1, len(hdrs_det)+1):
            c = ws.cell(ft,j); c.fill=xl_fill(CG); c.border=xl_brd(); c.font=xl_fn(bold=True)
        # SUMA de NÓMINA
        ws.cell(ft, COL_NOM).value  = f'=SUMA({LTR_NOM}{fila_ini}:{LTR_NOM}{fila_fin})'
        ws.cell(ft, COL_NOM).number_format  = '$#,##0.00'
        ws.cell(ft, COL_NOM).alignment = Alignment(horizontal="right")
        # SUMA de PDF (ignora vacíos)
        ws.cell(ft, COL_PDF).value  = f'=SUMA({LTR_PDF}{fila_ini}:{LTR_PDF}{fila_fin})'
        ws.cell(ft, COL_PDF).number_format  = '$#,##0.00'
        ws.cell(ft, COL_PDF).alignment = Alignment(horizontal="right")
        # DIFERENCIA total
        ws.cell(ft, COL_DIFF).value = f'={LTR_NOM}{ft}-{LTR_PDF}{ft}'
        ws.cell(ft, COL_DIFF).number_format = '$#,##0.00'
        ws.cell(ft, COL_DIFF).alignment = Alignment(horizontal="right")
        # Conteos
        col_est_ltr = LTR_EST
        ws.cell(ft, COL_EST).value = (
            f'=CONTAR.SI({col_est_ltr}{fila_ini}:{col_est_ltr}{fila_fin},"OK")'
            f'&" OK | "'
            f'&CONTAR.SI({col_est_ltr}{fila_ini}:{col_est_ltr}{fila_fin},"DIFIERE")'
            f'&" DIFIERE | "'
            f'&CONTAR.SI({col_est_ltr}{fila_ini}:{col_est_ltr}{fila_fin},"NO EN PDF")'
            f'&" NO EN PDF"'
        )
        ws.cell(ft, COL_EST).font = xl_fn(bold=True, size=8)
        ws.row_dimensions[ft].height = 22

        for i,w in enumerate([6,18,14,30,14,16,22,14,14,12,14,28,14,10,20,14,40,36,36],1):
            if i<=ws.max_column: ws.column_dimensions[get_column_letter(i)].width=w
        ws.freeze_panes="A5"  # congelar 4 filas (título+subtítulo+nota+header)
        fr=ws.max_row
...
<div class="kpi-row">
  ...
  <div class="kpi k-blue">
    ...
    <div class="kpi-s">Q1 + Q2</div>
        ...
        # ── Botón Excel arriba, junto a los KPIs ─────────────────────
        try:
            _xl_top = exportar_nomina_pdf_excel(
                cb["res"], cb["stats"], cb["dobles"], cb["tot_pdf"],
            ...
            st.warning(f"No se pudo generar Excel: {_e}")
  ...
  <span class="sec-desc">BBVA · Banorte · Santander</span>
    ...
    with st.expander("⚙  Configuración de periodo", expanded=True):
        ...
        with ct:
            ...
            tolerancia_tab3 = st.number_input("Tolerancia", value=1.0, min_value=0.0,
                                               step=0.5, label_visibility="collapsed",
                                               key="tol_tab3")

    # Rangos fijos — ya no se usan para filtrar, solo como referencia
    ...
    if PSYCOPG2_OK and usar_bd:
        ...
        st.markdown('<div class="up-lbl">Archivo de dispersión bancaria (CSV o Excel generado por extraer_pdfs.py)</div>', unsafe_allow_html=True)
        ups_banco = st.file_uploader("Dispersión bancaria", type=["csv","xlsx"],
                                      accept_multiple_files=True,
                                      key="up_banco_pdf", label_visibility="collapsed")
        if ups_banco:
            st.markdown(f'<div class="up-ok">📋 {len(ups_banco)} archivo(s) cargado(s)</div>',
                        unsafe_allow_html=True)

        if st.button("▶  Cargar nómina y archivo de dispersión",
                     type="primary", use_container_width=True, key="btn_bd"):
            if not ups_banco:
                st.warning("⬆ Primero sube el archivo CSV/Excel de dispersión")
            else:
                ...
                # ── Paso 2: leer CSV/Excel de dispersión ──────────
                pdf_rows_local = []
                errs_pdf = []
                bar = st.progress(0, "Leyendo archivo de dispersión…")
                for idx_f, f in enumerate(ups_banco):
                    try:
                        if f.name.lower().endswith(".csv"):
                            import csv as _csv
                            reader = _csv.DictReader(f.read().decode("utf-8-sig").splitlines())
                            for row in reader:
                                try:
                                    pdf_rows_local.append((
                                        row.get("estatus","") or row.get("banco",""),
                                        row.get("cuenta",""),
                                        float(row.get("importe",0) or 0),
                                        row.get("nombre",""),
                                        row.get("banco",""),
                                        row.get("archivo_pdf","") or f.name,
                                    ))
                                except: pass
                        elif f.name.lower().endswith(".xlsx"):
                            import openpyxl as _ox
                            wb_t = _ox.load_workbook(io.BytesIO(f.read()), data_only=True, read_only=True)
                            ws_t = wb_t.active
                            hdrs_t = [str(c or "").upper().strip() for c in next(ws_t.iter_rows(min_row=1,max_row=1,values_only=True))]
                            # Si la primera fila es título, buscar la de headers
                            if "CUENTA" not in hdrs_t:
                                hdrs_t = [str(c or "").upper().strip() for c in next(ws_t.iter_rows(min_row=2,max_row=2,values_only=True))]
                                min_r = 3
                            else:
                                min_r = 2
                            ci_banco  = next((i for i,h in enumerate(hdrs_t) if "BANCO" in h), None)
                            ci_cuenta = next((i for i,h in enumerate(hdrs_t) if "CUENTA" in h), None)
                            ci_imp    = next((i for i,h in enumerate(hdrs_t) if "IMPORTE" in h), None)
                            ci_nom    = next((i for i,h in enumerate(hdrs_t) if "NOMBRE" in h), None)
                            ci_arch   = next((i for i,h in enumerate(hdrs_t) if "ARCHIVO" in h), None)
                            for row_t in ws_t.iter_rows(min_row=min_r, values_only=True):
                                if not row_t or not any(row_t): continue
                                try:
                                    imp = float(row_t[ci_imp] or 0) if ci_imp is not None else 0
                                    if imp <= 0: continue
                                    pdf_rows_local.append((
                                        "",
                                        str(row_t[ci_cuenta] or "") if ci_cuenta is not None else "",
                                        imp,
                                        str(row_t[ci_nom] or "") if ci_nom is not None else "",
                                        str(row_t[ci_banco] or "") if ci_banco is not None else "",
                                        str(row_t[ci_arch] or f.name) if ci_arch is not None else f.name,
                                    ))
                                except: pass
                            wb_t.close()
                ...
                if not pdf_rows_local:
                    st.error("❌ No se encontraron registros. Verifica que sea el CSV/Excel generado por extraer_pdfs.py")
                else:
            ...
            if "df_sabanas" in st.session_state:
                ...
                if up_sabs:
                    st.markdown(f'<div class="up-ok">📎 {len(up_sabs)} archivo(s)</div>',
                                unsafe_allow_html=True)
        with cy:
            st.markdown('<div class="up-lbl">Archivo de dispersión (CSV o Excel de extraer_pdfs.py)</div>',
                        unsafe_allow_html=True)
        ...
        if st.button("▶  Preparar datos", use_container_width=True,
            ...
            # ── Leer archivo de dispersión (CSV o Excel) ──────────
            if not up_pdfs:
                st.warning("⬆ Sube el archivo CSV/Excel de dispersión bancaria"); st.stop()
            ...
            for idx3, f in enumerate(up_pdfs):
                try:
                    if f.name.lower().endswith(".csv"):
                        import csv as _csv3
                        reader = _csv3.DictReader(f.read().decode("utf-8-sig").splitlines())
                        for row in reader:
                            try:
                                imp = float(row.get("importe",0) or 0)
                                if imp <= 0: continue
                                pdf_rows_xl.append((
                                    row.get("estatus","") or row.get("banco",""),
                                    row.get("cuenta",""),
                        ...
                        wb_u = _ox2.load_workbook(io.BytesIO(f.read()), data_only=True, read_only=True)
                        ws_u = wb_u.active
                        hdrs_u = [str(c or "").upper().strip() for c in next(ws_u.iter_rows(min_row=1,max_row=1,values_only=True))]
                        if "CUENTA" not in hdrs_u:
                            hdrs_u = [str(c or "").upper().strip() for c in next(ws_u.iter_rows(min_row=2,max_row=2,values_only=True))]
                            min_r2 = 3
                        else:
                            min_r2 = 2
                        ci2 = {h: i for i,h in enumerate(hdrs_u)}
                        ci_b2 = next((i for h,i in ci2.items() if "BANCO" in h), None)
                        ci_c2 = next((i for h,i in ci2.items() if "CUENTA" in h), None)
                        ci_i2 = next((i for h,i in ci2.items() if "IMPORTE" in h), None)
                        ci_n2 = next((i for h,i in ci2.items() if "NOMBRE" in h), None)
                        ci_a2 = next((i for h,i in ci2.items() if "ARCHIVO" in h), None)
                        for row_u in ws_u.iter_rows(min_row=min_r2, values_only=True):
                            if not row_u or not any(row_u): continue
                            try:
                                imp = float(row_u[ci_i2] or 0) if ci_i2 is not None else 0
                                if imp <= 0: continue
                                pdf_rows_xl.append((
                                    "",
                                    str(row_u[ci_c2] or "") if ci_c2 is not None else "",
                                    imp,
                                    str(row_u[ci_n2] or "") if ci_n2 is not None else "",
                                    str(row_u[ci_b2] or "") if ci_b2 is not None else "",
                                    str(row_u[ci_a2] or f.name) if ci_a2 is not None else f.name,
                                ))
                            except: pass
                        wb_u.close()
                except Exception as ep:
            ...
            if not pdf_rows_xl:
                st.error("❌ Sin registros. Verifica que sea el archivo de extraer_pdfs.py"); st.stop()
