  ...
  <span class="sec-desc">Cédulas SUA · NSS · SDI · Días cotizados vs trabajados · Cuotas · Bajas</span>
    ...
    if hay_extracto:
        ...
        st.markdown(f'''<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;
          ...
          ✔ Extracto IMSS listo (Tab 01) — {n_ext:,} trabajadores únicos agrupados por NSS
        </div>''', unsafe_allow_html=True)
    elif hay_sabanas:
        ...
        st.markdown(f'''<div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;
          ...
          ⚠ Usando sábanas sin agrupar ({n_sab:,} filas) — para mejores resultados genera el
          <b>Extracto IMSS</b> con el botón del Tab 01.
        </div>''', unsafe_allow_html=True)
    else:
        st.warning("⬆ Procesa las sábanas en Tab 01 y genera el Extracto IMSS antes de continuar.")
    ...
    if ups_imss and st.button("▶  Extraer cédulas y cruzar", type="primary",
        ...
        if not hay_extracto and not hay_sabanas:
            ...
            # ── Paso 3: preparar nómina — preferir extracto sobre df_sabanas ──
            if "extracto_imss" in st.session_state:
                # Extracto ya viene agrupado por NSS con días sumados
                ...
                df_sab_grp = _ext.rename(columns={
                    "NOMBRE":         "nombre_sabana",
                    "SUELDO_MENSUAL": "sueldo_sabana",
                    "TOTAL_QUINCENAL":"total_quincenal",
                    "DIAS_TRABAJADOS":"dias_nom",
                    "BANCO":          "banco_sabana",
                    "N_ARCHIVOS":     "n_archivos",
                    "ARCHIVOS_ORIGEN":"archivos_origen",
                })
                df_sab_grp["dias_nom"] = pd.to_numeric(
                    df_sab_grp["dias_nom"], errors="coerce").fillna(0).astype(int)
                fuente_nom = f"Extracto IMSS ({len(df_sab_grp):,} trabajadores)"
            else:
                # Fallback: agrupar df_sabanas en el momento
                ...
                fuente_nom = f"Sábanas agrupadas ({len(df_sab_grp):,} trabajadores)"
            st.info(f"📋 Fuente nómina: {fuente_nom}")
