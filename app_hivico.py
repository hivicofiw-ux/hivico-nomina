...
def parsear_cedulas_imss(pdf_bytes, nombre_archivo):
    ...
    RE_EM    = _re.compile(r'^(\d{1,3})\s+(\d{1,3})\s+([\d,]+\.\d{2})')  # dias inc sdi...
        ...
        while i < len(todas_lineas):
            ...
            while j < len(todas_lineas) and j <= i + 3:
                ...
                if m_em and meta['tipo'] == 'EM':
                    ...
                    try: sdi = float(m_em.group(3).replace(',',''))
                    except: sdi = 0.0
                    nums  = [float(n.replace(',','')) for n in RE_NUM.findall(sig)]
                    vals  = [n for n in nums if n > 0.01]
                    # Estructura EM: SDI CF ExcPat ExcObr PDPat PDObr GMPPat GMPObr RT IVPat IVObr GPS Patronal Obrera SubTotal
                    if vals:
                        subtotal_em = vals[-1]  # SubTotal es el último
                        apt_pat = vals[-3] if len(vals) >= 3 else 0  # Patronal
...
<div class="sec-hdr">
  ...
  <span class="sec-desc">Cédulas SUA BIM + EM · HIVICO · FIREWALL · Comprobantes de pago</span>
</div>""", unsafe_allow_html=True)

    # ── 3 uploaders: HIVICO | FIREWALL | Comprobantes ─────────
    _ci1, _ci2, _ci3 = st.columns(3)
    with _ci1:
        st.markdown('''<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.4rem">
          <div style="width:12px;height:12px;border-radius:50%;background:#1A3A6B"></div>
          <span class="up-lbl" style="margin:0">Cédulas HIVICO</span></div>
        <div style="font-size:.62rem;color:#8A9BB0;margin-bottom:.3rem">BIM y/o EM · PDFs o ZIP</div>''',
                    unsafe_allow_html=True)
        ups_hiv = st.file_uploader("HIVICO", type=["pdf","zip"],
                                    accept_multiple_files=True,
                                    key="up_ced_hivico", label_visibility="collapsed")
        if ups_hiv:
            st.markdown(f'<div class="up-ok">📎 {len(ups_hiv)} archivo(s)</div>', unsafe_allow_html=True)

    with _ci2:
        st.markdown('''<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.4rem">
          <div style="width:12px;height:12px;border-radius:50%;background:#DC2626"></div>
          <span class="up-lbl" style="margin:0">Cédulas FIREWALL</span></div>
        <div style="font-size:.62rem;color:#8A9BB0;margin-bottom:.3rem">BIM y/o EM · PDFs o ZIP</div>''',
                    unsafe_allow_html=True)
        ups_fw = st.file_uploader("FIREWALL", type=["pdf","zip"],
                                   accept_multiple_files=True,
                                   key="up_ced_fw", label_visibility="collapsed")
        if ups_fw:
            st.markdown(f'<div class="up-ok">📎 {len(ups_fw)} archivo(s)</div>', unsafe_allow_html=True)

    with _ci3:
        st.markdown('''<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.4rem">
          <div style="width:12px;height:12px;border-radius:50%;background:#16A34A"></div>
          <span class="up-lbl" style="margin:0">Comprobantes de Pago</span></div>
        <div style="font-size:.62rem;color:#8A9BB0;margin-bottom:.3rem">Recibos bancarios SUA</div>''',
                    unsafe_allow_html=True)
        ups_comp = st.file_uploader("Comprobantes", type=["pdf","zip"],
                                     accept_multiple_files=True,
                                     key="up_comprobantes", label_visibility="collapsed")
        if ups_comp:
            st.markdown(f'<div class="up-ok">📎 {len(ups_comp)} archivo(s)</div>', unsafe_allow_html=True)

    # ── Configuración ──────────────────────────────────────────
    st.markdown('<div style="height:.4rem"></div>', unsafe_allow_html=True)
    _tc1, _tc2 = st.columns(2)
    with _tc1:
        tol_sdi  = st.slider("Tolerancia SDI (%)", 0, 20, 5, 1, key="tol_sdi_i4")
    with _tc2:
        tol_dias = st.slider("Tolerancia días", 0, 5, 2, 1, key="tol_dias_i4")

    # ── Estado fuente nómina ───────────────────────────────────
    ...
    if hay_extracto:
        ...
        st.markdown(f'''<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;
          padding:.6rem 1rem;font-size:.74rem;color:#15803D;font-weight:600;margin:.4rem 0">
          ✔ Extracto IMSS listo ({n_ext:,} trabajadores únicos) — Tab 01
        </div>''', unsafe_allow_html=True)
    elif hay_sabanas:
        st.markdown('''<div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;
          padding:.6rem 1rem;font-size:.74rem;color:#92400E;margin:.4rem 0">
          ⚠ Genera el <b>Extracto IMSS</b> en Tab 01 para mayor precisión
        </div>''', unsafe_allow_html=True)
    else:
        st.warning("⬆ Procesa las sábanas en Tab 01 primero.")

    ups_cedulas = (ups_hiv or []) + (ups_fw or [])

    if ups_cedulas and st.button("▶  Extraer cédulas y cruzar",
                                  type="primary", use_container_width=True, key="btn_imss"):
        if not hay_extracto and not hay_sabanas:
            st.error("❌ Sin datos de nómina. Procesa las sábanas en Tab 01.")
        else:
            # ── Paso 1: desempacar y parsear cédulas ──────────────
            pares_hiv = _pdfs_de_fuente(ups_hiv)
            pares_fw  = _pdfs_de_fuente(ups_fw)
            pares_comp= _pdfs_de_fuente(ups_comp)

            todos_regs = []; todos_meta = []
            bar_i = st.progress(0, "Parseando cédulas IMSS…")
            total_pdfs = len(pares_hiv) + len(pares_fw)

            for empresa_tag, pares in [("HIVICO", pares_hiv), ("FIREWALL", pares_fw)]:
                for nom_i, raw_i in pares:
                    regs_i, meta_i, err_i = parsear_cedulas_imss(raw_i, nom_i)
                    # Anotar empresa y tipo en cada registro
                    for r in regs_i:
                        r["empresa_tag"] = empresa_tag
                    todos_regs.extend(regs_i)
                    todos_meta.append({
                        "archivo"    : nom_i,
                        "empresa_tag": empresa_tag,
                        "n_regs"     : len(regs_i),
                        "reg_pat"    : meta_i.get("reg_patronal",""),
                        "empresa"    : meta_i.get("empresa",""),
                        "tipo"       : meta_i.get("tipo",""),
                        "bimestre"   : meta_i.get("bimestre",""),
                        "error"      : err_i,
                    })
                    bar_i.progress(len(todos_meta) / max(total_pdfs,1))
            bar_i.empty()

            # ── Paso 2: parsear comprobantes ───────────────────────
            comprobantes = []
            if pares_comp:
                bar_c = st.progress(0, "Leyendo comprobantes…")
                for idx_c, (nom_c, raw_c) in enumerate(pares_comp):
                    comp = parsear_comprobante_pago(raw_c, nom_c)
                    comprobantes.append(comp)
                    bar_c.progress((idx_c+1)/len(pares_comp))
                bar_c.empty()

            if not todos_regs:
                st.error("❌ Sin registros extraídos. Verifica que los PDFs sean de texto (SUA v3.7.x).")
                st.stop()

            df_imss = pd.DataFrame(todos_regs)
            df_imss["nss_norm"] = df_imss["nss"].astype(str).str.replace(r"\D","",regex=True)

            # Consolidar: si mismo NSS aparece en BIM y EM, fusionar — tomar días de BIM
            df_bim = df_imss[df_imss["tipo_cedula"]=="BIM"].copy()
            df_em  = df_imss[df_imss["tipo_cedula"]=="EM"].copy()

            # Dedup por NSS dentro de cada tipo (mayor suma_total primero)
            df_bim_u = df_bim.sort_values("suma_total",ascending=False).drop_duplicates("nss_norm",keep="first") if len(df_bim) else pd.DataFrame()
            df_em_u  = df_em.sort_values("suma_total",ascending=False).drop_duplicates("nss_norm",keep="first")  if len(df_em)  else pd.DataFrame()

            # Unir: preferir BIM para días/SDI, EM para cuotas EM
            if len(df_bim_u) and len(df_em_u):
                df_imss_u = df_bim_u.set_index("nss_norm").combine_first(
                    df_em_u.set_index("nss_norm")).reset_index()
            elif len(df_bim_u):
                df_imss_u = df_bim_u
            else:
                df_imss_u = df_em_u

            n_bim = int(df_imss["tipo_cedula"].eq("BIM").sum())
            n_em  = int(df_imss["tipo_cedula"].eq("EM").sum())
            st.info(f"📄 {len(df_imss):,} registros extraídos — BIM: {n_bim:,} · EM: {n_em:,} · únicos: {len(df_imss_u):,}")

            # ── Paso 3: preparar nómina ─────────────────────────────
            if hay_extracto:
                _ext = st.session_state["extracto_imss"].copy()
                _ext["nss_norm"] = _ext["NSS_NORM"].astype(str).str.replace(r"\D","",regex=True)
                df_sab_grp = _ext.rename(columns={
                    ...
                    "ARCHIVOS_ORIGEN":"archivos_origen","SERVICIO":"servicio",
                ...
                df_sab["dias_trabajados"] = pd.to_numeric(df_sab.get("dias_trabajados",0),errors="coerce").fillna(0)
                df_sab["sueldo_mensual"]  = pd.to_numeric(df_sab.get("sueldo_mensual",0),errors="coerce").fillna(0)
                df_sab["total_quincenal"] = pd.to_numeric(df_sab.get("total_quincenal",0),errors="coerce").fillna(0)
                df_sab_grp = df_sab.groupby("nss_norm",as_index=False).agg(
                    nombre_sabana=("nombre_completo","first"),
                    curp_sabana=("curp","first"),
                    empresa_sabana=("empresa","first"),
                    sueldo_sabana=("sueldo_mensual","max"),
                    total_quincenal=("total_quincenal","sum"),
                    dias_nom=("dias_trabajados","sum"),
                    banco_sabana=("banco","first"),
                    servicio=("nombre_servicio","first"),
                    n_archivos=("archivo_origen","count"),
                    archivos_origen=("archivo_origen",
                        lambda x: " | ".join(sorted(set(str(v) for v in x if v and str(v)!="nan")))),
                )

            df_sab_grp["dias_nom"] = pd.to_numeric(df_sab_grp.get("dias_nom",0),errors="coerce").fillna(0).astype(int)
            ...
            for _, ri in df_imss_u.iterrows():
                ...
                if n not in nss_sab:
                    filas.append({**ri.to_dict(),
                        "nombre_sabana":"","curp_sabana":"","empresa_sabana":"",
                        "sueldo_sabana":0,"total_quincenal":0,"dias_nom":0,
                        "servicio":"","n_archivos":0,"archivos_origen":"",
                        "dif_dias":None,"dif_sdi_pct":None,"estatus":"NO EN SÁBANA"})
                    continue
                ...
                elif dif_sdi > tol_sdi and abs(dif_dias) > tol_dias:
                    estatus = "SDI Y DÍAS DIFIEREN"
                elif dif_sdi > tol_sdi:
                    estatus = "SDI DIFIERE"
                elif abs(dif_dias) > tol_dias:
                ...
                filas.append({**ri.to_dict(),
                    ...
                    "curp_sabana":     rs.get("curp_sabana",""),
                    "empresa_sabana":  rs.get("empresa_sabana",""),
                    "sueldo_sabana":   sueldo_sab,
                    "total_quincenal": rs.get("total_quincenal",0),
                    "dias_nom":        dias_nom,
                    "servicio":        rs.get("servicio",""),
                    "n_archivos":      rs.get("n_archivos",0),
            ...
            for _, rs in df_sab_grp[~df_sab_grp["nss_norm"].isin(nss_imss)].iterrows():
                filas.append({
                    ...
                    "nombre":"","curp":"","clave_ubic":"","tipo_cedula":"","empresa_tag":"",
                    ...
                    "baja":False,"fecha_baja":"","fecha_mov":"","reg_patronal":"",
                    "bimestre":"","empresa_imss":"","archivo":"",
                    "nombre_sabana":   rs.get("nombre_sabana",""),
                    "curp_sabana":     rs.get("curp_sabana",""),
                    "empresa_sabana":  rs.get("empresa_sabana",""),
            ...
            st.success(f"✅  IMSS: {len(df_imss_u):,} únicos · Sábana: {len(df_sab_grp):,} · Cruce: {len(df_cruce):,}")

    # ── Mostrar resultados ─────────────────────────────────────
    if "cruce_imss" in st.session_state:
        ci    = st.session_state["cruce_imss"]
        df_cr = ci["df"]
        comps = ci.get("comprobantes",[])
...
<div class="kpi-row">
  <div class="kpi k-blue"><div class="kpi-v">{total:,}</div>
    ...
    <div class="kpi-s">IMSS: {ci["n_imss"]:,} · Nóm: {ci["n_sab"]:,}</div></div>
  ...
  <div class="kpi k-orange"><div class="kpi-v">{n_sdi:,}</div>
    <div class="kpi-l">SDI difiere</div><div class="kpi-s">Subdeclaración</div></div>
  ...
  <div class="kpi k-purple"><div class="kpi-v">{n_nsa:,}</div>
    <div class="kpi-l">Sin sábana</div><div class="kpi-s">En IMSS</div></div>
        ...
        if n_baja > 0:
            st.error(f"🔴  {n_baja:,} trabajador(es) con BAJA IMSS siguen en sábana")
...
<div class="sec-hdr" style="margin-top:1.2rem">
  ...
  <span class="sec-title">Cédulas procesadas</span>
        ...
        _log_html = ""
        for m in ci["meta"]:
            ok_ic = '<span style="color:#16A34A;font-weight:700">✔</span>' if not m["error"] else '<span style="color:#DC2626;font-weight:700">✘</span>'
            _tag_color = "#1A3A6B" if m["empresa_tag"]=="HIVICO" else "#DC2626"
            _tipo_bg   = "#EEF4FF" if m["tipo"]=="BIM" else "#FFF3E0"
            _tipo_color= "#1A3A6B" if m["tipo"]=="BIM" else "#92400E"
            _log_html += (
                f'<tr><td class="c">{ok_ic}</td>'
                f'<td class="mono" style="font-size:.7rem">{m["archivo"]}</td>'
                f'<td class="c"><span style="background:{_tag_color};color:white;padding:2px 7px;border-radius:4px;font-size:.6rem;font-weight:700">{m["empresa_tag"]}</span></td>'
                f'<td class="c"><span style="background:{_tipo_bg};color:{_tipo_color};padding:2px 7px;border-radius:4px;font-size:.6rem;font-weight:700">{m["tipo"]}</span></td>'
        ...
        st.markdown(f"""
<div class="t-wrap"><table class="t">
<thead><tr><th class="c"></th><th>Archivo</th><th class="c">Empresa</th>
<th class="c">Tipo</th><th>Reg. Patronal</th><th class="r">Trabajadores</th><th>Estado</th>
        ...
        if comps:
            st.markdown("""
<div class="sec-hdr" style="margin-top:1.2rem">
  <span class="sec-badge" style="background:#16A34A;color:white">✓</span>
  <span class="sec-title">Comprobantes de pago</span>
</div>""", unsafe_allow_html=True)
            _comp_html = ""
            for c in comps:
                _v = c.get("valido",False)
                _ic = '<span style="color:#16A34A;font-weight:700">✔</span>' if _v else '<span style="color:#DC2626;font-weight:700">✘</span>'
                _imp = f"${c.get('imp_total',0):,.2f}" if c.get("imp_total") else "—"
                _imss = f"${c.get('imp_imss',0):,.2f}" if c.get("imp_imss") else "—"
                _rcv  = f"${c.get('imp_rcv',0):,.2f}"  if c.get("imp_rcv")  else "—"
                _viv  = f"${c.get('imp_vivienda',0):,.2f}" if c.get("imp_vivienda") else "—"
                _comp_html += (
                    f'<tr><td class="c">{_ic}</td>'
                    f'<td class="mono" style="font-size:.7rem">{c.get("archivo","")}</td>'
                    f'<td style="font-size:.7rem">{c.get("empresa","")[:35]}</td>'
                    f'<td class="mono" style="font-size:.7rem">{c.get("reg_patronal","")}</td>'
                    f'<td style="font-size:.7rem">{c.get("folio_sua","")}</td>'
                    f'<td style="font-size:.7rem">{c.get("periodo","")}</td>'
                    f'<td class="r">{_imss}</td><td class="r">{_rcv}</td>'
                    f'<td class="r">{_viv}</td>'
                    f'<td class="r" style="font-weight:700;color:#1A3A6B">{_imp}</td>'
                    f'<td style="font-size:.7rem;color:#4A6A8A">{c.get("fecha_pago","")}</td>'
                    f'<td style="font-size:.7rem">{c.get("num_operacion","")}</td></tr>'
                )
            st.markdown(f"""
<div class="t-wrap"><table class="t">
<thead><tr><th class="c"></th><th>Archivo</th><th>Empresa</th><th>Reg.Pat.</th>
<th>Folio SUA</th><th>Período</th><th class="r">IMSS</th><th class="r">RCV</th>
<th class="r">Vivienda</th><th class="r">Total</th><th>Fecha</th><th>No.Oper.</th>
</tr></thead><tbody>{_comp_html}</tbody></table></div>""", unsafe_allow_html=True)

        # ── Excel ──────────────────────────────────────────────
        st.markdown("""
<div class="sec-hdr" style="margin-top:1.2rem">
  <span class="sec-badge" style="background:#1A3A6B">⬇</span>
  <span class="sec-title">Descargar reporte</span>
</div>""", unsafe_allow_html=True)
        try:
...
<div class="sec-hdr" style="margin-top:1.4rem">
  <span class="sec-badge" style="background:#F0F5FF;color:#1A3A6B">↓</span>
  <span class="sec-title">Detalle del cruce</span>
</div>""", unsafe_allow_html=True)
        ...
        with _fi1:
            f_est_i = st.multiselect("Estatus",
                ...
                key="fi_est_i4")
        with _fi2:
            f_emp_i = st.multiselect("Empresa cédula", ["HIVICO","FIREWALL"], key="fi_emp_i4")
        with _fi3:
            f_bus_i = st.text_input("Buscar NSS o nombre", key="fi_bus_i4")

        df_f = df_cr.copy()
        if f_est_i: df_f = df_f[df_f["estatus"].isin(f_est_i)]
        if f_emp_i: df_f = df_f[df_f.get("empresa_tag","").isin(f_emp_i)] if "empresa_tag" in df_f.columns else df_f
        if f_bus_i:
            _b = f_bus_i.lower()
        ...
        for _, r in df_f.head(500).iterrows():
            ...
            _etag_html = (f'<span style="background:{"#EEF4FF" if _etag=="HIVICO" else "#FFF0F0"};'
                          f'color:{"#1A3A6B" if _etag=="HIVICO" else "#DC2626"};'
                          f'padding:1px 5px;border-radius:3px;font-size:.58rem;font-weight:700">{_etag}</span>') if _etag else ""
            _arc  = str(r.get("archivos_origen",""))
            _arc_short = _arc[:40] + ("…" if len(_arc)>40 else "")
            ...
            _dd    = r.get("dif_dias")
            _dd_color = "#DC2626" if (_dd is not None and abs(_dd) > tol_dias) else "#16A34A"
            ...
            _rows_i += (
                f'<tr><td class="mono" style="font-size:.7rem">{r.get("nss_fmt") or r.get("nss","")}</td>'
                f'<td style="font-size:.75rem">{_nom} {_etag_html}</td>'
                f'<td style="font-size:.65rem;color:#5A7A9A">{_arc_short}</td>'
...
<tbody>{_rows_i}</tbody>
        ...
        if len(df_f) > 500:
            st.caption("500 mostrados. El Excel tiene el total completo.")
