...
def leer_layout_bytes(raw_bytes, nombre_archivo):
    ...
    try:
        ...
        if banco == 'BANORTE':
            ...
            hdr_row = 0
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]
            st.write(f"DEBUG BBVA | hdr_row={hdr_row} | hdr={hdr}")
            st.write(f"ci_cta={ci_cta} | ci_imp={ci_imp} | ci_nom={ci_nom}")
            for i, r in enumerate(rows[:10]):
                vals = [str(v or '').upper() for v in r]
                if any('CUENTA' in v or 'IMPORTE' in v for v in vals):
                    hdr_row = i; break
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]
            ci_cta  = next((i for i,h in enumerate(hdr) if 'CUENTA' in h or 'TDP' in h), 0)
            ci_imp  = next((i for i,h in enumerate(hdr) if 'IMPORTE' in h or 'NETO' in h), 1)
            ci_nom  = next((i for i,h in enumerate(hdr) if 'NOMBRE' in h), 2)
            for r in rows[hdr_row+1:]:
                if not r or not any(r): continue
            ...
            hdr_row = 0
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]
            st.write(f"DEBUG SANTANDER | hdr_row={hdr_row} | hdr={hdr}")
             st.write(f"ci_cta={ci_cta} | ci_imp={ci_imp}")
            for i, r in enumerate(rows[:10]):
                vals = [str(v or '').upper() for v in r]
                if any('CUENTA' in v or 'IMPORTE' in v or 'APELLIDO' in v for v in vals):
                    hdr_row = i; break
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]
            ci_num  = next((i for i,h in enumerate(hdr) if 'NUMERO' in h or 'NUM' in h), 0)
            ci_pat  = next((i for i,h in enumerate(hdr) if 'PATERNO' in h), 1)
            ci_mat  = next((i for i,h in enumerate(hdr) if 'MATERNO' in h), 2)
            ci_nom  = next((i for i,h in enumerate(hdr) if 'NOMBRE' in h and 'APELLIDO' not in h), 3)
            ci_cta  = next((i for i,h in enumerate(hdr) if 'CUENTA' in h), 4)
            ci_imp  = next((i for i,h in enumerate(hdr) if 'IMPORTE' in h), 5)
            for r in rows[hdr_row+1:]:
                if not r or not any(r): continue
