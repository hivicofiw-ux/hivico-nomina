...
def leer_layout_bytes(raw_bytes, nombre_archivo):
    """Lee layout de dispersión bancaria. Detecta banco por nombre de archivo.
    ...
    try:
        ...
        if banco == 'BANORTE':
            ...
            # Buscar fila header con "Empleado"
            hdr_row = 0
            for i, r in enumerate(rows[:10]):
                vals = [str(v or '').upper() for v in r]
                if any('EMPLEADO' in v for v in vals):
                    hdr_row = i; break
            ...
            for r in rows[hdr_row+1:]:
                ...
                # El importe viene con ceros: 000000000063008 → 630.08
                try:
                    imp_str = str(int(float(str(imp_raw).replace(',','') or '0'))).zfill(15)
                    imp = float(imp_str[:-2] + '.' + imp_str[-2:])
                except:
                    imp = _limpiar_imp_lay(imp_raw)
            ...
            # Buscar header — buscar hasta fila 15 con keywords ampliados
            hdr_row = 0
            for i, r in enumerate(rows[:15]):
                vals = [str(v or '').upper() for v in r]
                if any(k in v for v in vals for k in ['CUENTA','IMPORTE','MONTO','TDP','CLABE','BENEFICIARIO']):
                    hdr_row = i; break
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]

            ci_cta = next((i for i,h in enumerate(hdr)
                           if any(k in h for k in ['CUENTA','TDP','CLABE','TARJETA'])), None)
            ci_imp = next((i for i,h in enumerate(hdr)
                           if any(k in h for k in ['IMPORTE','NETO','MONTO','CANTIDAD','TOTAL'])), None)
            ci_nom = next((i for i,h in enumerate(hdr)
                           if any(k in h for k in ['NOMBRE','BENEFICIARIO'])), None)

            # Fallback: si no se detectaron columnas por header, escanear filas de datos
            # y detectar por tipo de valor (cuenta = 10-18 dígitos, importe = float)
            if ci_cta is None or ci_imp is None:
                for r in rows[hdr_row+1:hdr_row+6]:
                    if not r or not any(r): continue
                    for j, v in enumerate(r):
                        sv = str(v or '').strip().replace(',','').replace('$','')
                        if ci_cta is None and re.match(r'^\d{10,18}$', re.sub(r'\.0$','',sv)):
                            ci_cta = j
            ...
            # Guardia: ci_imp no puede ser igual a ci_cta
            if ci_imp == ci_cta:
                ci_imp = ci_cta + 1

            st.info(f"DEBUG BBVA | hdr_row={hdr_row} | headers={hdr} | ci_cta={ci_cta} ci_imp={ci_imp} ci_nom={ci_nom}")
            ...
            # Cols: Num.Empleado | Ap.Paterno | Ap.Materno | Nombre | CUENTA | IMPORTE | ID_Concepto
            hdr_row = 0
            for i, r in enumerate(rows[:15]):
                vals = [str(v or '').upper() for v in r]
                if any(k in v for v in vals for k in ['CUENTA','IMPORTE','MONTO','APELLIDO','PATERNO','CLABE']):
                    hdr_row = i; break
            hdr = [str(v or '').upper().strip() for v in rows[hdr_row]]

            ci_num = next((i for i,h in enumerate(hdr)
                           if any(k in h for k in ['NUMERO','NUM','ID EMPLEADO','EMPLEADO'])), None)
            ...
            ci_imp = next((i for i,h in enumerate(hdr)
                           if any(k in h for k in ['IMPORTE','MONTO','NETO','CANTIDAD','TOTAL'])), None)

            # Fallback: si ci_cta o ci_imp no se detectaron, escanear datos
            if ci_cta is None or ci_imp is None:
                for r in rows[hdr_row+1:hdr_row+6]:
                    if not r or not any(r): continue
                    for j, v in enumerate(r):
            ...
            st.info(f"DEBUG SANTANDER | hdr_row={hdr_row} | headers={hdr} | ci_cta={ci_cta} ci_imp={ci_imp}")
...
<div class="sec-hdr">
  ...
  <span class="sec-desc">BBVA · Banorte · Santander</span>
    ...
    st.markdown('<div class="up-lbl">Sube los layouts (XLS, XLSX o CSV)</div>', unsafe_allow_html=True)
    ups_lay = st.file_uploader("Layouts", type=["xls","xlsx","csv"],
                                accept_multiple_files=True, key="up_layouts",
                                label_visibility="collapsed")
    if ups_lay:
        st.markdown(f'<div class="up-ok">📎 {len(ups_lay)} archivo(s)</div>', unsafe_allow_html=True)

    if ups_lay and st.button("▶  Analizar Layouts", type="primary",
                              use_container_width=True, key="btn_lay"):
        _regs_tmp, _log_tmp = [], []
        _bar = st.progress(0)
        for _i, _f in enumerate(ups_lay):
            _r, _t, _e = leer_layout_bytes(_f.read(), _f.name)
            _regs_tmp.extend(_r); _log_tmp.append((_f.name, len(_r), _t, _e))
            _bar.progress((_i+1)/len(ups_lay))
        _bar.empty()
