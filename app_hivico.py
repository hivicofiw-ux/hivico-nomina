...
def _detectar_banco_contenido(rows):
    ...
    contenido = ' '.join(
        str(v or '').upper()
        for r in rows[:20] for v in (r if r else [])
    )
    # SANTANDER: apellidos separados
    if 'PATERNO' in contenido and 'MATERNO' in contenido:
        return 'SANTANDER'
    # BANORTE: tiene EMPLEADO en header
    if 'EMPLEADO' in contenido:
        return 'BANORTE'
    # BBVA: tiene TDP o BENEFICIARIO
    if 'TDP' in contenido or 'BENEFICIARIO' in contenido:
        return 'BBVA'
    # BBVA fallback: CUENTA + IMPORTE en misma fila
    for r in rows[:15]:
...
def _imp_banorte(imp_raw):
    ...
    y formato decimal normal (1234.56). Detecta automáticamente cuál es."""
    ...
    try:
        ...
        return 0.0
    # Si ya tiene punto decimal en el string original → formato decimal normal
    if '.' in s:
        return f
    # Si es entero grande (>= 6 dígitos) → formato ceros: últimos 2 son centavos
    iv = int(f)
    if iv >= 100:  # mínimo $1.00 en formato ceros = 100
        s2 = str(iv).zfill(15)
        converted = float(s2[:-2] + '.' + s2[-2:])
        # Sanidad: si el resultado parece razonable (< $500k por empleado) úsalo
