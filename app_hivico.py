...
def parsear_cedulas_imss(pdf_bytes, nombre_archivo):
    """Extrae registros de trabajadores de cédula SUA IMSS.
    Devuelve (lista_registros, meta_dict, error_str)"""
    try:
        import pdfplumber as _plb
        import re as _re

        RE_NSS    = _re.compile(r'(\d{2}-\d{2}-\d{2}-\d{4}-\d)')
        RE_CURP   = _re.compile(r'([A-Z]{4}[-]?\d{6}[A-Z]{6,9}\d{2}|[A-Z]{4}-\d{6}-)')
        RE_FECHA  = _re.compile(r'\d{2}/\d{2}/\d{4}')
        RE_NUM    = _re.compile(r'[\d,]+\.\d{2}')
        RE_REG_PAT= _re.compile(r'Registro\s+Patronal[:\s]+([A-Z0-9\-]+)')
        RE_BIMEST = _re.compile(r'Bimestre\s+de\s+Proceso[:\s]+([A-Za-záéíóúÁÉÍÓÚ\-]+\-?\d{4})')
        RE_EMPRESA= _re.compile(r'Nombre\s+o\s+Raz[oó]n\s+Social[:\s]+(.+)')

        meta = {'reg_patronal': '', 'bimestre': '', 'empresa': '', 'archivo': nombre_archivo}
        registros = []

        with _plb.open(io.BytesIO(pdf_bytes)) as pdf:
            texto_todo = ''
            for pag in pdf.pages:
                t = pag.extract_text() or ''
                texto_todo += t + '\n'

        # Extraer meta del encabezado (primeras líneas)
        for linea in texto_todo.split('\n')[:30]:
            if not meta['reg_patronal']:
                m = RE_REG_PAT.search(linea)
                if m: meta['reg_patronal'] = m.group(1).strip()
            if not meta['bimestre']:
                m = RE_BIMEST.search(linea)
                if m: meta['bimestre'] = m.group(1).strip()
            if not meta['empresa']:
                m = RE_EMPRESA.search(linea)
                if m: meta['empresa'] = m.group(1).strip()[:80]

        # Parsear líneas de trabajadores
        lineas = texto_todo.split('\n')
        i = 0
        while i < len(lineas):
            linea = lineas[i].strip()

            # Buscar línea con NSS
            m_nss = RE_NSS.search(linea)
            if not m_nss:
                i += 1
                continue

            nss_raw = m_nss.group(1)
            nss     = nss_raw.replace('-', '')

            # Extraer CURP y nombre del resto de la línea
            resto = linea[m_nss.end():].strip()
            curp  = ''
            nombre = ''
            clave_ubic = ''

            m_curp = RE_CURP.search(resto)
            if m_curp:
                curp = m_curp.group(1).strip()
                # Nombre: lo que hay entre NSS y CURP, o después del CURP
                antes  = resto[:m_curp.start()].strip()
                despues= resto[m_curp.end():].strip()
                nombre = antes if antes else despues
                # Clave ubicación: últimas 4 chars si son alfanum tipo IM21
                partes = despues.split()
                if partes and _re.match(r'^[A-Z]{2}\d{2}$|^[A-Z]{2,4}\d{2}$|^[A-Z]{2}\d{2}[A-Z]?$', partes[-1]):
                    clave_ubic = partes[-1]
                    nombre     = nombre or ' '.join(partes[:-1])
            else:
                nombre = resto

            nombre = nombre.strip()
            if nombre.upper() in ('', 'NAN', 'NONE'): nombre = ''

            # Buscar línea ISM/ISV/etc. (siguiente línea no vacía)
            dias = 0; sdi = 0.0
            cuota_patronal = 0.0; cuota_obrera = 0.0; suma_cv = 0.0
            apt_patronal = 0.0; amortizacion = 0.0; suma_total = 0.0
            fecha_mov = ''; baja = False; fecha_baja = ''

            j = i + 1
            while j < len(lineas) and j < i + 4:
                sig = lineas[j].strip()
                if _re.match(r'^(ISM|ISV|ISB|P/IV|P/CV|S/R|J/R|E/C)\b', sig):
                    nums = RE_NUM.findall(sig)
                    fechas = RE_FECHA.findall(sig)
                    if fechas: fecha_mov = fechas[0]
                    # Días: primer entero corto después de la fecha o al inicio
                    m_dias = _re.search(r'\b(\d{1,3})\b', sig[sig.find(fecha_mov)+10 if fecha_mov in sig else 3:])
                    if m_dias:
                        try: dias = int(m_dias.group(1))
                        except: pass
                    # SDI: suele ser el primer número con decimales ~300-1500
                    for n in nums:
                        try:
                            v = float(n.replace(',',''))
                            if 100 <= v <= 5000 and sdi == 0.0:
                                sdi = v
                                break
                        except: pass
                    # Asignar cuotas por posición en lista de nums
                    floats = []
                    for n in nums:
                        try: floats.append(float(n.replace(',','')))
                        except: pass
                    # Buscar el grupo de cuotas: patronal ≈ obrera*3 (aproximado)
                    # Estrategia: los primeros 3 floats razonables (~100-8000) son
                    # patronal_cv, obrera_cv, suma_cv; luego apt_patronal
                    razonables = [v for v in floats if 10 <= v <= 100000]
                    if len(razonables) >= 4:
                        # patronal C&V, obrera C&V, suma C&V, apt_patronal
                        cuota_patronal = razonables[0]
                        cuota_obrera   = razonables[1]
                        suma_cv        = razonables[2]
                        apt_patronal   = razonables[3]
                    if len(razonables) >= 6:
                        amortizacion = razonables[4]
                        suma_total   = razonables[5]
                    elif len(razonables) >= 5:
                        suma_total   = razonables[4]
                    break
                j += 1

            # Buscar Baja en siguientes líneas
            for k in range(i+1, min(i+5, len(lineas))):
                if lineas[k].strip().lower().startswith('baja'):
                    baja = True
                    m_fb = RE_FECHA.search(lineas[k])
                    if m_fb: fecha_baja = m_fb.group(0)
                    break

            if not nss or len(nss) < 8:
                i += 1
                continue

            # Salario mensual implícito (SDI * 30)
...
<div class="sec-hdr">
  ...
  <span class="sec-desc">Cédulas SUA · NSS · SDI · Cuotas · Bajas</span>
</div>""", unsafe_allow_html=True)

    # ── Uploader cédulas ───────────────────────────────────────
    st.markdown('<div class="up-lbl">Cédulas IMSS (PDF — SUA v3.7.x, uno o varios)</div>',
                unsafe_allow_html=True)
    ups_imss = st.file_uploader("Cédulas IMSS", type=["pdf"],
                                 accept_multiple_files=True,
                                 key="up_cedulas_imss",
                                 label_visibility="collapsed")
    if ups_imss:
        st.markdown(f'<div class="up-ok">📎 {len(ups_imss)} cédula(s) cargada(s)</div>',
                    unsafe_allow_html=True)
    ...
    with _ci1:
        tol_sdi = st.slider("Tolerancia SDI (%)", 0, 20, 5, 1,
                             help="Diferencia máxima % entre SDI·30 e IMSS vs sueldo sábana para considerar OK")
    with _ci2:
        st.markdown(f"""
<div style="background:#EEF4FF;border:1px solid #C0D4F0;border-radius:8px;padding:.7rem 1rem;margin-top:.3rem">
  <div style="font-size:.6rem;font-weight:700;color:#4A90D9;letter-spacing:1.5px;text-transform:uppercase">¿Qué se cruza?</div>
  <div style="font-size:.7rem;color:#3A4A5A;margin-top:.3rem;line-height:1.6">
    ✔ NSS presente en ambas fuentes<br>
    ✔ SDI·30 vs sueldo mensual sábana (±{tol_sdi}%)<br>
    ✔ Trabajadores sin IMSS / sin sábana<br>
    ✔ Bajas IMSS que siguen en sábana
  </div>
</div>""", unsafe_allow_html=True)
    ...
    if hay_sabanas:
        ...
        st.markdown(f'<div class="up-ok" style="margin:.4rem 0">✔ Sábanas del Tab 01 disponibles — {n_sab:,} empleados</div>',
                    unsafe_allow_html=True)
    else:
        st.warning("⬆ Primero procesa las sábanas en el Tab 01 para tener la nómina disponible.")
    ...
    if ups_imss and st.button("▶  Extraer cédulas y cruzar", type="primary",
        ...
        if not hay_sabanas:
            ...
            # ── Paso 1: parsear cédulas ──────────────────────────
            todos_regs = []; todos_meta = []; errs_imss = []
            bar_i = st.progress(0, "Extrayendo cédulas IMSS…")
            for idx_i, f_i in enumerate(ups_imss):
                regs_i, meta_i, err_i = parsear_cedulas_imss(f_i.read(), f_i.name)
                if err_i:
                    errs_imss.append(f"⚠ {f_i.name}: {err_i[:120]}")
                todos_regs.extend(regs_i)
                todos_meta.append((f_i.name, len(regs_i), meta_i.get('reg_patronal',''),
                                    meta_i.get('empresa_imss',''), err_i))
                bar_i.progress((idx_i + 1) / len(ups_imss))
            bar_i.empty()

            for e in errs_imss: st.warning(e)

            if not todos_regs:
                st.error("❌ No se extrajeron registros de las cédulas. Verifica que sean PDFs de texto (no escaneados).")
            else:
                df_imss = pd.DataFrame(todos_regs)
                # Normalizar NSS: solo dígitos
                df_imss['nss_norm'] = df_imss['nss'].astype(str).str.replace(r'\D','',regex=True)

                # ── Paso 2: preparar sábana ──────────────────────
                df_sab = st.session_state["df_sabanas"].copy()
                df_sab['nss_norm'] = df_sab['nss'].astype(str).str.replace(r'\D','',regex=True)
                df_sab['sueldo_mensual'] = pd.to_numeric(df_sab.get('sueldo_mensual', 0),
                                                          errors='coerce').fillna(0)
                # Agrupar sábana por NSS (puede aparecer en varias sábanas)
                df_sab_grp = (df_sab.groupby('nss_norm', as_index=False)
                              .agg(nombre_sabana=('nombre_completo','first'),
                                   sueldo_sabana=('sueldo_mensual','max'),
                                   banco_sabana=('banco','first'),
                                   archivo_sabana=('archivo_origen','first')))

                # Deduplicate IMSS por NSS (quedarse con el de mayor suma_total)
                df_imss_grp = (df_imss.sort_values('suma_total', ascending=False)
                               .drop_duplicates('nss_norm', keep='first'))

                # ── Paso 3: cruce ────────────────────────────────
                nss_imss = set(df_imss_grp['nss_norm'].dropna())
                nss_sab  = set(df_sab_grp['nss_norm'].dropna())

                filas = []

                # En ambos
                for _, ri in df_imss_grp.iterrows():
                    n = ri['nss_norm']
                    if n not in nss_sab:
                        filas.append({**ri.to_dict(),
                                      'nombre_sabana':'', 'sueldo_sabana':0,
                                      'banco_sabana':'','archivo_sabana':'',
                                      'estatus':'NO EN SÁBANA', 'dif_sdi_pct':None})
                        continue
                    ...
                    if sueldo_sab > 0 and sueldo_ims > 0:
                        dif_pct = abs(sueldo_sab - sueldo_ims) / sueldo_sab * 100
                    else:
                        dif_pct = 0.0
                    if ri.get('baja'):
                        estatus = 'BAJA CON PAGO'
                    elif dif_pct > tol_sdi:
                        estatus = 'SDI DIFIERE'
                    else:
                        estatus = 'OK'
                    filas.append({**ri.to_dict(),
                                  ...
                                  'archivo_sabana':rs.get('archivo_sabana',''),
                                  'estatus': estatus,
                                  'dif_sdi_pct': round(dif_pct, 1)})

                # En sábana pero no en IMSS
                for _, rs in df_sab_grp[~df_sab_grp['nss_norm'].isin(nss_imss)].iterrows():
                    filas.append({'nss_norm': rs['nss_norm'],
                                  'nss': rs['nss_norm'], 'nss_fmt': rs['nss_norm'],
                                  'nombre_imss':'', 'curp':'', 'clave_ubic':'',
                                  'dias':0,'sdi':0,'sueldo_imss':0,
                                  'cuota_patronal':0,'cuota_obrera':0,
                                  'suma_cv':0,'apt_patronal':0,'amortizacion':0,'suma_total':0,
                                  ...
                                  'banco_sabana':  rs.get('banco_sabana',''),
                                  'archivo_sabana':rs.get('archivo_sabana',''),
                                  'estatus':'NO EN IMSS','dif_sdi_pct':None})

                df_cruce = pd.DataFrame(filas)
                st.session_state["cruce_imss"] = {
                    "df": df_cruce, "meta": todos_meta,
                    "n_imss": len(df_imss_grp), "n_sab": len(df_sab_grp)
                }
                st.success(f"✅  IMSS: {len(df_imss_grp):,} trabajadores  ·  Sábana: {len(df_sab_grp):,}  ·  Cruce: {len(df_cruce):,} registros")
    ...
    if "cruce_imss" in st.session_state:
        ...
        n_sdi  = len(df_cr[df_cr['estatus'] == 'SDI DIFIERE'])
...
<div class="kpi-row">
  ...
  <div class="kpi k-red"><div class="kpi-v">{n_nim:,}</div>
    ...
    <div class="kpi-s">En sábana, no en cédula</div></div>
  <div class="kpi k-purple"><div class="kpi-v">{n_nsa:,}</div>
    ...
    <div class="kpi-s">En IMSS, no en nómina</div></div>
        ...
        # Bajas con pago alert
        if n_baja > 0:
            st.error(f"🔴  {n_baja:,} trabajador(es) con BAJA en IMSS aparecen en sábana — revisar urgente")
...
<div class="sec-hdr" style="margin-top:1.2rem">
  ...
  <span class="sec-title">Cédulas procesadas</span>
        ...
        for arch, n_r, reg_pat, empresa, err in ci["meta"]:
            ok_ic = '<span style="color:#16A34A;font-weight:700">✔</span>' if not err else '<span style="color:#DC2626;font-weight:700">✘</span>'
            _log_html += (f'<tr><td class="c">{ok_ic}</td>'
                          f'<td class="mono">{arch}</td>'
                          f'<td style="font-size:.71rem;color:#4A6A8A">{reg_pat}</td>'
                          f'<td style="font-size:.71rem;color:#4A6A8A">{empresa[:40]}</td>'
                          f'<td class="r">{n_r:,}</td>'
                          f'<td style="font-size:.7rem;color:{"#DC2626" if err else "#16A34A"}">{err[:60] if err else "OK"}</td></tr>')
        st.markdown(f"""
<div class="t-wrap"><table class="t">
<thead><tr><th class="c"></th><th>Archivo</th><th>Reg. Patronal</th>
<th>Empresa</th><th class="r">Trabajadores</th><th>Estado</th></tr></thead>
<tbody>{_log_html}</tbody></table></div>""", unsafe_allow_html=True)
...
<div class="sec-hdr" style="margin-top:1.4rem">
  ...
  <span class="sec-title">Detalle de registros</span>
        ...
        with _fi1:
            f_est_i = st.multiselect("Estatus", ["OK","SDI DIFIERE","NO EN IMSS",
                                                   "NO EN SÁBANA","BAJA CON PAGO"],
                                      default=["SDI DIFIERE","NO EN IMSS","NO EN SÁBANA","BAJA CON PAGO"],
                                      key="fi_est")
        with _fi2:
            f_baja_i = st.checkbox("Solo bajas IMSS con pago", key="fi_baja")
        with _fi3:
            f_bus_i = st.text_input("Buscar NSS o nombre", placeholder="Escriba para filtrar…", key="fi_bus")
        ...
        if f_bus_i:
            _b = f_bus_i.lower()
            df_f = df_f[df_f.apply(lambda r: _b in str(r.get('nss','')+r.get('nombre_sabana','')+r.get('nombre_imss','')).lower(), axis=1)]
        ...
        _COLOR_EST = {"OK":"b-ok","SDI DIFIERE":"b-dif","NO EN IMSS":"b-no",
                      "NO EN SÁBANA":"b-dif","BAJA CON PAGO":"b-no"}
        _rows_i = ""
        for _, r in df_f.head(500).iterrows():
            _est = r.get('estatus','')
            _bc  = _COLOR_EST.get(_est, "")
            _sdi_s = f"${r['sdi']:,.2f}" if r.get('sdi') else '<span style="color:#B0BCC8">—</span>'
            _sue_s = f"${r.get('sueldo_sabana',0):,.2f}" if r.get('sueldo_sabana') else '<span style="color:#B0BCC8">—</span>'
            _dif_s = (f"{r['dif_sdi_pct']:.1f}%" if r.get('dif_sdi_pct') is not None
                      else '<span style="color:#B0BCC8">—</span>')
            _baja_s = '<span style="color:#DC2626;font-weight:700">⚠ BAJA</span>' if r.get('baja') else ''
            _nombre = r.get('nombre_sabana') or r.get('nombre_imss','')
            _rows_i += (f'<tr><td class="mono">{r.get("nss_fmt") or r.get("nss","")}</td>'
                        f'<td style="color:#2A3A4A">{str(_nombre)[:38]}</td>'
                        f'<td class="r">{_sdi_s}</td>'
                        f'<td class="r">{_sue_s}</td>'
                        f'<td class="r">{_dif_s}</td>'
                        f'<td class="c">{r.get("dias","")}</td>'
                        f'<td>{_baja_s}</td>'
...
<div class="t-wrap"><table class="t">
<thead><tr>
  <th>NSS</th><th>Nombre</th><th class="r">SDI IMSS</th>
  <th class="r">Sueldo Sábana</th><th class="r">Dif %</th>
  <th class="c">Días</th><th>Baja</th><th>Estatus</th>
</tr></thead>
<tbody>{_rows_i}</tbody>
</table></div>""", unsafe_allow_html=True)
